...
This module will introduce you to concepts and approaches to working with high-dimensional data, such as gene expression data. We will focus on dimension reduction techniques, i.e. moving from a high-dimensional context to a lower-dimensional context, and the development of predictive models using .

## A. Overfitting Demonstrated
Let's pretend there is a gene whose expresion serves as a biomarker of whether the tissue sampled is cancerous or normal tissue, where high expression indicates greater liklihood of cancer. Below, we simulate this condition by sampling from two 'gaussian' distribution, one that is non-cancer (normal) and one that is cancer. Because there is only 1 gene, this is a 1-dimensional problem. The optimal dividing line between cancer & non-cancer is not perfect, but separates vast majority of samples between the two groups.
...
clin_df$ageGroups <- factor(cut(clin_df$age_at_diagnosis, breaks=c(0, 40, 75, 100)))
...
There were several 'decisions' that were made in the modeling approach above in the form of modeling parameters. This is often referred to as hyper-parameters. What were some of these choices? What were these hyper-parameters? Would strategies could we take to make more informed decisions about these hyper-parameters?
